...
def api_base_url():
    ...
    配置优先级：
    1. 环境变量 EXAMPLE_SERVICE_API_URL
    2. .env 文件中的 EXAMPLE_SERVICE_API_URL
    3. 自动检测 Docker 端口映射
    ...
    # 优先级 1: 环境变量
    api_url = os.getenv("EXAMPLE_SERVICE_API_URL")
    if api_url:
...
# ============================================================
# @trace 主追溯 marker 收集期校验
# (复制自 .claude/skills/qa/assets/trace_framework.py —— skill = 规范源,本项目 = 实例化)
# 用法见 .claude/skills/qa/references/test_traceability.md
# ============================================================
from trace_framework import validate_trace_items  # noqa: E402


def pytest_collection_modifyitems(config, items):
    """收集期校验 @trace 标注(定位符互斥 + ac 枚举 + design 格式)。

    默认 warning;TRACE_STRICT=1 升为收集 fail。
    """
