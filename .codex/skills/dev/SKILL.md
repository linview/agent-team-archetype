...
| **目录命名** | `service_event_watcher`, `scripts_database` | `service-event-watcher`, `scripts-database` |
...
# 示例：重命名目录
git mv sandbox/service-event-watcher sandbox/service_event_watcher
```
...
> ```bash
> # 添加到 ~/.bashrc 或 ~/.zshrc
> echo 'source $(git rev-parse --show-toplevel)/.codex/skills/dev/scripts/git-workflow.skill.sh' >> ~/.bashrc
> source ~/.bashrc
> ```
>
> **快速使用**：
...
**SKILL 文档**：
- `.codex/skills/qa/SKILL.md` - QA 工作流程和测试规范
- `.codex/skills/pm/SKILL.md` - Scrum 工作流程
- `.codex/skills/arch/SKILL.md` - 架构设计规范
