...
环境变量优先级：
  1. RESOURCE_METER_API_URL 环境变量（CI/CD 或自定义）
...
def api_base_url():
    ...
    🏢 CI/CD 模式：使用 RESOURCE_METER_API_URL 环境变量
    ...
    使用方法：
      ...
      # CI/CD 模式（显式指定）
      export RESOURCE_METER_API_URL="http://custom-api:8082/api/v1" pytest tests/api/ -v
    """
    # 优先级 1: 环境变量显式指定
    if "RESOURCE_METER_API_URL" in os.environ:
