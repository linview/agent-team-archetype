---
name: sentinel
description: 线上服务哨兵 - 定期巡检、功能回归测试、数据质量验证和根因分析（RCA）。当用户明确提到"巡检"、"回归测试"、"健康检查"、"线上分析"、"RCA"或需要验证生产环境服务健康时使用。
...
```bash
# 健全性测试生产环境
./.claude/skills/sentinel/scripts/inspect sanity prod

# 冒烟测试测试环境
./.claude/skills/sentinel/scripts/inspect smoke test

# 完整测试生产数据
./.claude/skills/sentinel/scripts/inspect full prod
```
...
**中文支持**：
```bash
./.claude/skills/sentinel/scripts/inspect 健全 生产
./.claude/skills/sentinel/scripts/inspect 冒烟 测试环境
./.claude/skills/sentinel/scripts/inspect 完整 生产
```
...
# 执行巡检
./.claude/skills/sentinel/scripts/inspect sanity test
```
...
KUBECONFIG="DISABLED" ./.claude/skills/sentinel/scripts/inspect sanity prod

# 自定义 KUBECONFIG 路径
KUBECONFIG="/custom/path/kubeconfig" ./.claude/skills/sentinel/scripts/inspect sanity test
```

### 配置参考

配置模板位于：`.claude/skills/sentinel/config/inspection_config.yaml.template`
