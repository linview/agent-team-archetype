...
#
# Stop Hook 触发脚本：在 Claude session 结束时启动 spec-xchecker
#
# 用法:
...
# 启动独立 CC Session（后台执行，120秒超时）
timeout 120s nohup claude -p "/spec-xchecker --auto-mode --output-format json --output ${OUTPUT_DIR}/report.json" \
  > "${OUTPUT_DIR}/cc_session.log" 2>&1 &
